...
def train(opt):
    ...
    if 'CTC' in opt.Prediction:
        if opt.baiduCTC:
            ...
            from warpctc_pytorch import CTCLoss # https://github.com/espnet/espnet/issues/1699
            criterion = CTCLoss()
        else:
        ...
        if (iteration + 1) % opt.valInterval == 0 or iteration == 0: # To see training progress, we also conduct validation when 'iteration == 0' 
            ...
            with open(f'./saved_models/{opt.exp_name}/log_train.txt', 'a') as log:
                ...
                for gt, pred, confidence in zip(labels[:5], preds[:5], confidence_score[:5]):
                    ...
                    # print("gt",gt)
                    # print("pred",pred)
                    # print("predicted_result_log",predicted_result_log)
                predicted_result_log += f'{dashed_line}'
...
if __name__ == '__main__':
    ...
    train(opt)
#python train.py --train_data data_lmdb/train --valid_data data_lmdb/validation --Transformation TPS --FeatureExtraction ResNet --SequenceModeling BiLSTM --Prediction CTC --data_filtering_off --workers 0 --imgH 64 --imgW 200
#batch_max_length 50으로 바꾸는 것도 괜찮을듯 유미네헤어살롱을 유미네헤어살까지 밖에 인식이 안되는데
# 이는 아마 텍스트 길이 제한 때문일듯 생각